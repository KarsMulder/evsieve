...
fn main() {
    ...
    if cfg!(feature = "trace_shutdown") {
        println!("Info: exit code interpreted.");
    }
    daemon::await_completion();
    if cfg!(feature = "trace_shutdown") {
        println!("Info: daemon notification completed.");
    }
    subprocess::terminate_all();
    if cfg!(feature = "trace_shutdown") {
        println!("Info: SIGTERM sent to all subprocesses.");
    }
    if cfg!(feature = "trace_shutdown") {
        println!("Info: evsieve shall exit now.");
    }
...
fn handle_ready_file(program: &mut Program, index: FileIndex) -> Result<Action, RuntimeError> {
    ...
    match file {
        ...
        Pollable::SignalFd(fd) => {
            if cfg!(feature = "trace_shutdown") {
                println!("Info: signalfd ready.");
            }
            let siginfo = fd.read_raw()?;
            let signal_no = siginfo.ssi_signo as i32;
            if cfg!(feature = "trace_shutdown") {
                println!("Info: signal {} received.", signal_no);
            }
            if TERMINATION_SIGNALS.contains(&signal_no) {
                if cfg!(feature = "trace_shutdown") {
                    println!("Info: termination signal confirmed.");
                }
                Ok(Action::Exit)
            } else {
