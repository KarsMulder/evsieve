...
pub enum PollMessage {
    ...
    Ready,
    /// A file signaled ERR or HUP.
    Broken,
    /// Nothing special is going on with this file.
    Waiting,
}

/// A wrapper around the more traditional poll() system call. Experiments show that it has virtually the
/// same performance as epoll() for our use case, but it's API is sometimes more convenient to use.
///
/// Takes a list of files. Waits until at least one of those files can be read from, or breaks.
/// Returns an iterator over one message for each file.
///
/// There is a one-to-one correspondence between the files passed into this function and the messages
/// generated, and those messages are given in the same order as the list of files given to this function.
pub fn poll<'a, T: AsRawFd + 'a> (files: impl IntoIterator<Item=&'a T>)
        -> Result<impl Iterator<Item=PollMessage>, SystemError>
{
    let mut poll_fds: Vec<libc::pollfd> = files.into_iter().map(|file| {
        libc::pollfd {
            fd: file.as_raw_fd(),
            events: libc::POLLIN,
            revents: 0,
    ...
    if result < 0 {
        ...
        Ok(poll_fds.into_iter().map(|poll_fd| {
            if poll_fd.revents & libc::POLLERR != 0 || poll_fd.revents & libc::POLLHUP != 0 {
                PollMessage::Broken
            } else if poll_fd.revents & libc::POLLIN != 0 {
                PollMessage::Ready
            } else {
                // TODO: Check for POLLNVAL?
                PollMessage::Waiting
            }
        }))
    }
